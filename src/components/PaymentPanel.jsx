...
export default function PaymentPanel({
  ...
  const showCashValidation = paymentMethod === 'Tunai' && cashReceived.length > 0;
  const canSubmit =
    paymentMethod !== 'Tunai' || (cashReceived.length > 0 && isCashValid);
      ...
      <div className="mt-4 space-y-3">
        ...
        {paymentMethod === 'Tunai' && (
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
            ...
            {showCashValidation && !isCashValid && (
              <p className="mt-1 text-xs font-medium text-rose-500">
                Uang belum cukup untuk total {formatCurrency(total)}.
              </p>
            )}
          </div>
        )}
